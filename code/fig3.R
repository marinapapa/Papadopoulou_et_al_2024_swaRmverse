...
p3A <- ggplot2::ggplot(
  ...
  ggplot2::theme(legend.position = 'top',legend.justification = 'center',
                 ...
                 axis.text = ggplot2::element_text(color = 'black', size = 12, family = 'Trebuchet MS'),
...
##########
## Fig. 3B - Relative position of neighbors in goats
...
p3B <- ggplot2::ggplot(data_df[data_df$nnd < 5 & data_df$nnd > 0.1,], ## clean out outliers for visuals
...
cowplot::plot_grid(p3A, p3B, labels = 'AUTO')
